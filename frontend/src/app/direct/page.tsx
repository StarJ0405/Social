...
import { createRoom, deleteAlarm, fetchUser, getAlarms, getRooms, saveArticleTempImage, writeArticle } from '../Global/API/UserAPI';
...
export default function Home() {
    ...
    const [alarms, setAlarms] = useState([] as any[]);
    ...
    useEffect(() => {
        if (ACCESS_TOKEN) {
            fetchUser()
                .then((response) => {
                    setUser(response);
                    getAlarms().then(r => setAlarms(r)).catch(e => console.log(e));
    ...
    function transferTime(long: number) {
        ...
        let month = '';
        if (year || date.getMonth() != now.getMonth())
            month = (date.getMonth() + 1).toString();
        let day = '';
        if (month || date.getDate() != now.getDate())
            day = date.getDate().toString();

        return (year ? year + "-" : '') + (month ? month + "-" : '') + (day ? day + " " : '') + ((date.getHours() > 12 ? '오후 ' + (date.getHours() - 12) : '오전 ' + date.getHours()) + ":" + date.getMinutes() + ":" + date.getSeconds())
    }
    function Profile() {
        return (
            ...
            <div className={'fixed sidebar h-screen flex w-[500px] z-[5]'}>
                <div className={'h-screen border-r-2 flex flex-col justify-start w-[120px]'}>
                    ...
                    <Modal open={isModalOpen} onClose={() => {
                        const article_temp_img = document.getElementById('article_temp_img');
                                        ...
                                        <div className='w-full h-full felx flex-col p-2'>
                                            ...
                                            <div>
                                                <div className='flex flex-col'>
                                                    <textarea id='text' className='w-full h-[500px] resize-none outline-none overflow-y-scroll' maxLength={2200} placeholder='문구를 입력하세요...' onKeyDown={(e: any) => { const value = e.target.value as string; setContent(value ? value : ''); }} onKeyUp={(e: any) => { const value = e.target.value as string; setContent(value ? value : ''); }}></textarea>
                                ...
                                <div className={'bg-base-100 h-screen border-r-2 flex flex-col justify-start w-full'}>
                                    ...
                                    <div className='flex flex-col self-center items-center w-[300px] text-sm text-center'>
                                        {alarms.length == 0 ?
                                            ...
                                            :
                                            alarms.map((alarm, index) => <div key={index} className='flex justify-between w-full my-2'>
                                                <a href={alarm.link ? alarm.link : null} className={alarm.link ? 'hover:underline' : ''}>{alarm.message}</a>
                                                <div className='flex flex-col items-end' onClick={() => console.log(alarm.id)}>
                                                    <img src='/commons/x.png' className='w-[12px] h-[12px] cursor-pointer' onClick={() => deleteAlarm({ id: alarm.id }).then(r => setAlarms(r)).catch(e => console.log(e))} />
                                                    <label className='self-end'>{transferTime(alarm.createDate)}</label>
                                                </div>
                                            </div>)
                                        }
                                    </div>
                    ...
                    !room ?
                        ...
                        <div className='flex flex-col relative' id='m_pbg'>
                            ...
                            <div className='flex items-center rounded-full border border-black mx-4 px-4' style={{ height: 44 + 'px' }}>
                                ...
                                <input type="text" autoFocus id='message_text' className='outline-none ml-2 break-words w-full resize-none' placeholder='메시지 입력...' onKeyDown={e => {
                                    if (e.key == "Enter")
                                        document.getElementById('send')?.click();
                                }} />
                                <label id="send" className='min-w-[55px] h-[18px] text-blue-500 hover:text-black cursor-pointer' onClick={Send}>보내기</label>
                            </div>
                        </div>
                }
            </div>
